# encoding: utf-8
from urllib2 import Request as rs
...
def find_Doc(account,status,find_times=100):
    ...
    print('start:' + (time.time() - start_time).__str__())
    logging.info('开始寻找医生,%s'%account)
    j = 0
    while True:
        j += 1
        page = rs.urlopen(state_center)
        print('urlclose:'+(time.time()-start_time).__str__())
        try:
            hjson = json.loads(page.read())
            State = hjson[account]['State']
            logging.debug('Account:%s,status:%s' %(account,State))
            print(('Account:%s,status:%s' %(account,State)))
        except Exception as e:
            State = -1
            logging.debug('没有找到医生,Account:%s,error:%s'%(account,e))
            print(('没有找到医生,Account:%s,error:%s'%(account,e)))
        if(State==status or j>=find_times):
            logging.debug('找到医生或超时退出寻找,轮训次数:%d,Account:%s'%(j,account))
            print('find times:'+(time.time() - start_time).__str__())
            print(('找到医生或超时退出寻找,轮训次数:%d,Account:%s'%(j,account)))
            break
        time.sleep(0.01)
